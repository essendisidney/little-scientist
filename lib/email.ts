...
/** Notify info@ — never throws; logs on failure. */
export async function sendInfoEmail({
  ...
  if (!smtpConfigured()) {
    console.warn('sendInfoEmail skipped: SMTP not configured')
  ...
  try {
    ...
    console.error('sendInfoEmail failed', err instanceof Error ? err.message : err)
    return { sent: false, reason: 'send_failed' }
  }
}

/** Guest-facing email (enquiry ack, etc.). Never throws. */
export async function sendGuestEmail({
  ...
  const addr = to.trim()
  if (!addr.includes('@')) return { sent: false, reason: 'invalid_to' }
  ...
  try {
    ...
    console.error('sendGuestEmail failed', err instanceof Error ? err.message : err)
    return { sent: false, reason: 'send_failed' }
  }
}
