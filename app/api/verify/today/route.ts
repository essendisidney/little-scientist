...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin
    .from('tickets')
    .select(
      'qr_code, is_used, used_at, ticket_type, bookings(booking_ref, booker_name, adult_count, child_count, infant_count, payment_status, sessions(session_date, time_slot))',
    )
